@echo off
echo.
echo Importing table message to Oracle
...
rem 	message.bat table

rem To import data into the table run:
rem 	message.bat import
...
sqlplus eureka_streams/YourekaTent!@SW2_GUILLEMOT @message_ddl.sql
...
sqlldr userid=eureka_streams/YourekaTent!@SW2_GUILLEMOT control=message.ctl log=message.log rows=1000 readsize=1048576 bindsize=1048576



echo Creating a sequence and trigger to implement the identity column

sqlplus eureka_streams/YourekaTent!@SW2_GUILLEMOT @message_id.sql
